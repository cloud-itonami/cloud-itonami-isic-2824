(ns heavyequip.operation
  "OperationActor -- one mining/quarrying/construction-machinery-
  manufacturing operation = one supervised actor run, expressed as a
  langgraph-clj StateGraph. The advisor (Heavy Equipment Advisor) is
  sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Heavy Equipment Governor (:govern) and the rollout phase
  ...
  One graph run = one mining/quarrying/construction-machinery-
  manufacturing operation (intake -> advise -> govern -> decide ->
  commit | hold | approval). No unbounded inner loop -- each
  ...
  decision to a human operator (the heavy-equipment manufacturing
  engineer). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:actuation/dispatch-unit`/
  `:actuation/issue-stability-certificate` ALWAYS reach this node when
  the governor is clean -- see `heavyequip.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [heavyequip.heavyequipadvisor :as heavyequipadvisor]
...
(defn build
  ...
  `heavyequip.store/Store`).
  opts:
    :advisor      -- a `heavyequip.heavyequipadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (heavyequipadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Heavy Equipment Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (heavyequipadvisor/-advise advisor store request)]
            {:proposal p :audit [(heavyequipadvisor/trace request p)]})))

      ;; Heavy Equipment Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
