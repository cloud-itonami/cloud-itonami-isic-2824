(ns heavyequip.sim
  ...
  intake -> requirements verification -> stability/braking-test
  screening -> unit-dispatch proposal (always escalates) -> human
  approval -> commit, then through stability-certificate proposal
  (always escalates) -> human approval -> commit, then shows five
  HARD holds (a jurisdiction with no spec-basis, an out-of-spec
  stability margin, an unresolved stability/braking-test defect
  screened directly via `:stability-brake-test/screen` [never via an
  actuation op against an unscreened unit -- see this actor's own
  ...
  dispatch/stability-certificate-issuance of an already-processed
  unit) that never reach a human at all, and prints the audit ledger
  + the draft unit-dispatch and stability-certificate records."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== unit/intake unit-1 (JPN, clean; stability-margin within spec, no stability/brake defect) ==")
    ...
    (println "== stability-brake-test/screen unit-1 (clean; escalates -- human approves) ==")
    (println (exec! actor "t3" {:op :stability-brake-test/screen :subject "unit-1"} operator))
    ...
    (let [r (exec! actor "t4" {:op :actuation/dispatch-unit :subject "unit-1"} operator)]
      (println r)
      (println "-- human heavy-equipment manufacturing engineer approves --")
      (println (approve! actor "t4")))

    (println "== actuation/issue-stability-certificate unit-1 (always escalates -- actuation/issue-stability-certificate) ==")
    (let [r (exec! actor "t5" {:op :actuation/issue-stability-certificate :subject "unit-1"} operator)]
      (println r)
      (println "-- human heavy-equipment manufacturing engineer approves --")
    ...
    (println "== actuation/dispatch-unit unit-3 (0.35 outside [-0.10,0.10] stability margin -> HARD hold) ==")
    (println (exec! actor "t8" {:op :actuation/dispatch-unit :subject "unit-3"} operator))

    (println "== stability-brake-test/screen unit-4 (unresolved -> HARD hold, never reaches a human) ==")
    (println (exec! actor "t9" {:op :stability-brake-test/screen :subject "unit-4"} operator))
    ...
    (println "== actuation/issue-stability-certificate unit-1 AGAIN (double-issuance -> HARD hold) ==")
    (println (exec! actor "t11" {:op :actuation/issue-stability-certificate :subject "unit-1"} operator))
    ...
    (println "== draft stability-certificate records ==")
