(ns heavyequip.store-contract-test
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      (is (= "Sakura Hydraulic Excavator EX-220" (:unit-name (store/unit s "unit-1"))))
      (is (= "JPN" (:jurisdiction (store/unit s "unit-1"))))
      (is (= 0.05 (:stability-margin-actual (store/unit s "unit-1"))))
      (is (= -0.10 (:stability-margin-min (store/unit s "unit-1"))))
      (is (= 0.10 (:stability-margin-max (store/unit s "unit-1"))))
      (is (false? (:stability-brake-test-defect-unresolved? (store/unit s "unit-1"))))
      (is (= 0.35 (:stability-margin-actual (store/unit s "unit-3"))))
      (is (true? (:stability-brake-test-defect-unresolved? (store/unit s "unit-4"))))
      (is (false? (:unit-dispatched? (store/unit s "unit-1"))))
      (is (false? (:stability-certified? (store/unit s "unit-1"))))
      (is (= ["unit-1" "unit-2" "unit-3" "unit-4"]
             (mapv :id (store/all-units s))))
      (is (nil? (store/stability-brake-screen-of s "unit-1")))
    ...
    (testing label
      (testing "partial upsert merges, preserving untouched fields"
        (store/commit-record! s {:effect :unit/upsert
                                 :value {:id "unit-1" :unit-name "Sakura Hydraulic Excavator EX-220"}})
        (is (= "Sakura Hydraulic Excavator EX-220" (:unit-name (store/unit s "unit-1"))))
        (is (= 0.05 (:stability-margin-actual (store/unit s "unit-1"))) "unrelated field preserved"))
      (testing "verification / stability-brake-screen payloads commit and read back"
        ...
        (store/commit-record! s {:effect :stability-brake-screen/set :path ["unit-1"]
                                 :payload {:unit-id "unit-1" :verdict :resolved}})
        (is (= {:unit-id "unit-1" :verdict :resolved} (store/stability-brake-screen-of s "unit-1"))))
      (testing "unit dispatch drafts a record and advances the sequence"
        ...
        (is (= "JPN-HEQ-000000" (get (first (store/dispatch-history s)) "record_id")))
      ...
      (testing "stability certificate drafts a record and advances the sequence"
        (store/commit-record! s {:effect :unit/mark-certified :path ["unit-1"]})
        (is (= "JPN-STB-000000" (get (first (store/evidence-history s)) "record_id")))
        (is (= "stability-certificate-draft" (get (first (store/evidence-history s)) "kind")))
        (is (true? (:stability-certified? (store/unit s "unit-1"))))
...
(deftest datomic-empty-store-is-usable
  (let [s (store/datomic-store)]
    ...
    (is (zero? (store/next-dispatch-sequence s "JPN")))
    (is (zero? (store/next-evidence-sequence s "JPN")))
    (store/with-units s {"x" {:id "x" :unit-name "n" :stability-margin-actual 0.05
                                   :stability-margin-min -0.10 :stability-margin-max 0.10
